...
COIN_DAEMON='/usr/local/bin/foxcoind'
COIN_CLI='/usr/local/bin/foxcoin-cli'
COIN_REPO='https://www.dropbox.com/preview/foxcoind-linux-daemon.zip'
COIN_NAME='FOXCOIN'
COIN_PORT=25676
...
#Generating Random Password for foxcoind RPC
...
if grep -q "SwapTotal" /proc/meminfo; then
    ...
    touch /var/swap.img
    chmod 600 swap.img
    dd if=/dev/zero of=/var/swap.img bs=1M count=4000
...
function compile_node() {
  echo -e "FOXCOIN AUTO MASTERNODE INSTALLATION SCRIPT"
...
function create_key() {
  echo -e "Enter your ${RED}$COIN_NAME Masternode Private Key${NC}. Or Press Enter To Auto Generate New Private Key for ${RED}$COIN_NAME "
  read -e COINKEY
  if [[ -z "$COINKEY" ]]; then
  $COIN_DAEMON -daemon
  sleep 30
...
function enable_firewall() {
  echo -e "Installing and setting up firewall to allow connections on port ${YELLOW}$COIN_PORT${NC}"
...
function important_information() {
 ...
 echo -e "Check if $COIN_NAME is running by using the following command: ${RED}systemctl status FOXCOIN.service${NC}"
 echo -e "================================================================================================================================"
}
